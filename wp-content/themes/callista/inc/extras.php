 ...
 * @return string
 */
 function callista_set_post_meta_value( $value ) {
 	$queried_obj = apply_filters( 'callista_queried_object_id', false );
 	if ( ! $queried_obj ) {
 	...
 	$curr_opions = 'callista_' . str_replace( 'theme_mod_', '', current_filter() );
  ...
  * Sidebar position
  */
 add_filter( 'theme_mod_sidebar_position', 'callista_set_post_meta_value' );

 /**
  * Header container type
  */
 add_filter( 'theme_mod_header_container_type', 'callista_set_post_meta_value' );

 /**
  * Content container type
  */
 add_filter( 'theme_mod_content_container_type', 'callista_set_post_meta_value' );
  /**
  * Footer container type
  */
 add_filter( 'theme_mod_footer_container_type', 'callista_set_post_meta_value' );
 ...
 * @return string
 */
function callista_render_macros( $string ) {

	$macros = apply_filters( 'callista_data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function callista_render_icons( $content ) {
	$icons     = callista_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'callista_render_icons_callback', $content );
}
 ...
 * @return string
 */
function callista_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function callista_get_render_icons_set() {
	return apply_filters( 'callista_render_icons_set', array(
 ...
 * @return string
 */
function callista_render_theme_url( $url ) {
 ...
 */
function callista_get_image_id_by_url( $image_src ) {
	global $wpdb;
...
function callista_post_formats_gallery() {
	$size = callista_post_thumbnail_size();
	...
	$images = callista_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images(false);
 ...
 * @return bool
 */
function callista_is_meta_visible( $meta, $context = 'loop' ) {

	if ( ! $meta ) {
		return false;
	}

	$meta_enabled = get_theme_mod( $meta, callista_theme()->customizer->get_default( $meta ) );
 ...
 * @return array
 */
function callista_post_thumbnail_size( $args = array() ) {
	$args = wp_parse_args( $args, array(
		'small'        => 'post-thumbnail',
		'fullwidth'    => 'callista-thumb-l',
		'class_prefix' => '',
	) );

	$layout      = get_theme_mod( 'blog_layout_type', callista_theme()->customizer->get_default( 'blog_layout_type' ) );
	$format      = get_post_format();
	$size_option = get_theme_mod( 'blog_featured_image', callista_theme()->customizer->get_default( 'blog_featured_image' ) );
