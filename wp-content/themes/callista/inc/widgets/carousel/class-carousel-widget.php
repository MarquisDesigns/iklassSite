 ...
 * @package Callista
 */
if ( ! class_exists( 'Callista_Carousel_Widget' ) ) {

	class Callista_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description	= esc_html__( 'Display a list of your posts on your site.', 'callista' );
			$this->widget_id			= 'callista_widget_carousel';
			$this->widget_name			= esc_html__( 'Carousel', 'callista' );
			...
			$this->settings				= array(
				...
				'slides_per_view' => array(
					...
					'min_value'			=> 1,
					'value'				=> 5,
					'label'				=> esc_html__( 'Number of slides per view', 'callista' ),
				),
				'slides_per_group' => array(
					...
					'min_value'			=> 1,
					'value'				=> 1,
					'label'				=> esc_html__( 'Number slides per group', 'callista' ),
				),
				'slides_per_column' => array(
					...
					'min_value'			=> 1,
					'value'				=> 1,
					'label'				=> esc_html__( 'Multi Row Slides Layout', 'callista' ),
				),
				'space_between_slides' => array(
					...
					'min_value'			=> 0,
					'value'				=> 30,
					'label'				=> esc_html__( 'Width of the space between slides(px)', 'callista' ),
				),
				'duration_speed' => array(
					...
					'min_value'			=> 100,
					'value'				=> 500,
					'label'				=> esc_html__( 'Duration of transition between slides (ms)', 'callista' ),
				),
				'navigation' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Slider navigation', 'callista' ),
				),
				'pagination' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Slider pagination', 'callista' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', callista_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="callista-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'callista' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'callista_carousel_default_query_args', array(
				'post_type'			=> 'post',
				'orderby'			=> 'date',
				'order'				=> 'DESC',
				'posts_per_page'	=> -1,
		...
		public function get_carousel_loop( $posts_query ) {
			...
			if ( ! $view_dir ) {
				return '<h5>' . esc_html__( 'View file not found', 'callista' ) . '</h5>';
			}
	...
	add_action( 'widgets_init', 'callista_register_carosel_widgets' );
	function callista_register_carosel_widgets() {
		register_widget( 'Callista_Carousel_Widget' );
	}
}
