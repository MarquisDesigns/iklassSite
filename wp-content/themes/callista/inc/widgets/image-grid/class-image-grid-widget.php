 ...
 * @package Callista
 */
if ( ! class_exists( 'Callista_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class Callista_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name			= esc_html__( 'Image Grid', 'callista' );
			$this->widget_description	= esc_html__( 'This widget displays images from post.', 'callista' );
			...
			$this->settings				= array(
				...
				'category_name' => array(
					...
					'label'				=> esc_html__( 'Select categories to show', 'callista' ),
				...
				'post_sort' => array(
					...
					'value'				=> 'date',
					'options'			=> array(
						'date' 			=> esc_html__( 'Publish Date', 'callista' ),
						'title'			=> esc_html__( 'Post Title', 'callista' ),
						'comment_count'	=> esc_html__( 'Comment Count', 'callista' ),
					),
					'label'				=> esc_html__( 'Post sorted', 'callista' ),
				),
				'post_number' => array(
					'type'				=> 'stepper',
					'value'				=> '5',
					...
					'min_value'			=> '1',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Posts number', 'callista' ),
				),
				'post_offset' => array(
					'type'				=> 'stepper',
					'value'				=> '0',
				...
				'columns_number' => array(
					...
					'min_value'			=> '1',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Columns number', 'callista' ),
				),
				'items_padding' => array(
					'type'				=> 'stepper',
					'value'				=> '5',
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'callista_image_grid_widget_args', $args, $instance );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if( $post_taxonomy ){
					$post_args = array(
						...
						'offset'		=> $post_offset,
						'orderby'		=> $post_sort,
						'order'			=> apply_filters( 'callista_order_image_grid_widget', 'DESC', $instance ),//ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				echo apply_filters( 'callista_image_grid_widget_before', '<div class="row columns-number-' . $columns_number . '" ' . $row_inline_style . '>', $instance );
				...
				echo apply_filters( 'callista_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'callista_register_image_grid_widget' );
	function callista_register_image_grid_widget() {
		register_widget( 'Callista_Image_Grid_Widget' );
	}
