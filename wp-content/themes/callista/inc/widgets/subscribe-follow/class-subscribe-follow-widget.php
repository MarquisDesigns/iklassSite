...
class Callista_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'callista' );
		$this->widget_id          = 'callista_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'callista' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'callista' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'callista' ),
				'label' => esc_html__( 'Subscribe Title', 'callista' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'callista' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your e-mail', 'callista' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'callista' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'callista' ),
				'label' => esc_html__( 'Subscribe submit label', 'callista' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'callista' ),
				'label' => esc_html__( 'Subscribe success', 'callista' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'callista' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'callista' ),
				'label' => esc_html__( 'Follow Title', 'callista' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'callista' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'callista' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'callista' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'callista' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'callista' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'callista' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'callista' ),
					'top-center'    => esc_html__( 'Top Center', 'callista' ),
					'top-right'     => esc_html__( 'Top Right', 'callista' ),
					'center-left'   => esc_html__( 'Middle Left', 'callista' ),
					'center'        => esc_html__( 'Middle Center', 'callista' ),
					'center-right'  => esc_html__( 'Middle Right', 'callista' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'callista' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'callista' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'callista' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'callista' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'callista' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'callista' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'callista' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'callista' ),
					'contain' => esc_html__( 'Contain', 'callista' ),
					'auto'    => esc_html__( 'Auto', 'callista' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'callista' ),
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_callista_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_callista_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'callista' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'callista' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'callista' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'callista' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'callista' );
		}
	...
	public function get_social_nav() {
		return callista_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = callista_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'callista_subscribe' ) ) {
...
add_action( 'widgets_init', 'callista_register_subscribe_follow_widgets' );
function callista_register_subscribe_follow_widgets() {
	register_widget( 'Callista_Subscribe_Follow_Widget' );
}
