...
if ( ! class_exists( 'Healtro_Instagram_Widget' ) ) {
	...
	class Healtro_Instagram_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of photos from Instagram network.', 'callista' );
			$this->widget_id          = 'callista_widget_instagram';
			$this->widget_name        = esc_html__( 'Instagram', 'callista' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'Instagram', 'callista' ),
					'label' => esc_html__( 'Title', 'callista' ),
				),
				'tag' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Hashtag (enter without `#` symbol)', 'callista' ),
				),
				'image_counter' => array(
					'type'       => 'stepper',
					'value'      => '6',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Number of photos', 'callista' ),
				),
				'display_caption' => array(
					...
					'options' => array(
						'display_caption_check' => esc_html__( 'Caption', 'callista' ),
					),
				),
				'display_date' => array(
					'type'  => 'checkbox',
					...
					'options' => array(
						'display_date_check' => esc_html__( 'Date', 'callista' ),
					),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( empty( $instance['tag'] ) ) {
				return print $args['before_widget'] . esc_html__( 'Please, enter #hashtag.', 'callista' ) . $args['after_widget'];
			}
			...
			if ( ! $photos ) {
				return print $args['before_widget'] . esc_html__( 'No photos. Maybe you entered a invalid hashtag.', 'callista' ) . $args['after_widget'];
			}

			$this->widget_start( $args, $instance );

			printf( '<div class="%s">',
				join( ' ', apply_filters( 'callista_instagram_widget_wrapper_class', array( 'instagram__items' ) ) )
			);
		...
		public function get_photos( $data, $img_counter, $config ) {
			 ...
			 * @var string
			 */
			$order = apply_filters( 'callista_instagram_widget_order', 'media' );
			...
			foreach ( $nodes as $photo ) {
				...
				if ( in_array( 'caption', $config ) ) {
					$_photo['caption'] = wp_html_excerpt(
						$photo['caption'],
						apply_filters( 'callista_instagram_widget_caption_length', 10 ),
						apply_filters( 'callista_instagram_widget_caption_more', '&hellip;' )
					);
				}
		...
		public function get_tags_url() {
			return apply_filters( 'callista_instagram_widget_get_tags_url', $this->service_url . 'explore/tags/%s/' );
		}
		...
		public function get_post_url() {
			return apply_filters( 'callista_instagram_widget_get_post_url', $this->service_url . 'p/%s/' );
		}
	...
	add_action( 'widgets_init', 'callista_register_instagram_widget' );
	function callista_register_instagram_widget() {
