 ...
 */
add_filter( 'template_include', array( 'Callista_Wrapping', 'wrap' ), 99 );
add_filter( 'callista_wrap_base', 'callista_wrap_base_cpts' );
 ...
 */
function callista_template_path() {
	return Callista_Wrapping::$main_template;
}
 ...
 */
function callista_template_base() {
	return Callista_Wrapping::$base;
}

if ( ! class_exists( 'Callista_Wrapping' ) ) {
	...
	class Callista_Wrapping {
		...
		/**
		 * Initialize new instance of Callista_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		public function __toString() {
			$this->templates = apply_filters( 'callista_wrap_' . $this->slug, $this->templates );
		 ...
		 * and $base as static variables in Callista_Wrapping class.
		 *
		 * @since 1.0.0
 ...
 */
function callista_wrap_base_cpts( $templates ) {
